...
import { useStore, type TranslationDirection } from '../../store/useStore';

const directions: { value: TranslationDirection; label: string }[] = [
  { value: 'assembly-to-machine', label: 'Assembly → Machine Code' },
  { value: 'machine-to-assembly', label: 'Machine Code → Assembly' },
];

export function TranslationDirectionSelector() {
  const { translationDirection, setTranslationDirection } = useStore();

  return (
    <Listbox value={translationDirection} onChange={setTranslationDirection}>
      <div className="relative mt-1">
        <Listbox.Button className="relative w-full cursor-default rounded-lg bg-white dark:bg-gray-700 py-2 pl-3 pr-10 text-left shadow-md focus:outline-none focus-visible:border-indigo-500 focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75 focus-visible:ring-offset-2 focus-visible:ring-offset-orange-300 sm:text-sm text-gray-900 dark:text-white">
          <span className="block truncate">{translationDirection === 'assembly-to-machine' ? 'Assembly to Machine Code' : 'Machine Code to Assembly'}</span>
